CREATE TABLE OZU_EMPLOYEES (
  ...
  TOTAL_WORK_PERCENTAGE NUMBER       NOT NULL,
  MEMBERSHIP_COUNT      NUMBER(1),
  DEPARTMENT_ID         NUMBER(4),
  CONSTRAINT "OZU_EMP_SALARY_MIN" CHECK (salary > 0) ENABLE,
  CONSTRAINT "OZU_MEMBER_MAX" CHECK (MEMBERSHIP_COUNT <= 5 AND MEMBERSHIP_COUNT >= 0) ENABLE,
  CONSTRAINT "OZU_WORK_PER_MAX" CHECK (TOTAL_WORK_PERCENTAGE <= 1 AND TOTAL_WORK_PERCENTAGE >= 0) ENABLE,
...
CREATE TABLE OZU_PROJECT_TEAMS (
  ...
  CHECK ("WORK_PERCENTAGE" < 1 AND "WORK_PERCENTAGE" > 0) ENABLE
);
...
CREATE TABLE OZU_TASKS (
  ...
  TASK_END_DATE   DATE,
  PROJECT_ID      NUMBER(4)   NOT NULL,
  COORDINATOR_ID  NUMBER(6)   NOT NULL,
...
/* EMP_TASKS TABLE FK */
ALTER TABLE OZU_EMP_TASKS
  ADD CONSTRAINT "OZU_EMP_TASKS_EMP_ID_FK"
    FOREIGN KEY ("EMPLOYEE_ID")
    REFERENCES "OZU_EMPLOYEES"("EMPLOYEE_ID") ENABLE;

ALTER TABLE OZU_EMP_TASKS
  ADD CONSTRAINT "OZU_EMP_TASKS_TASK_ID_FK"
    FOREIGN KEY ("TASK_ID")
