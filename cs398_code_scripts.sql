...
BEGIN
  IF NOT DELETING THEN
    ...
    BEGIN
      SELECT DISTINCT COUNT(EMPLOYEE_ID) FROM OZU_PROJECT_TEAMS
    ...
    ELSE
      BEGIN
        SELECT DISTINCT COUNT(EMPLOYEE_ID) FROM OZU_PROJECT_TEAMS
        WHERE PROJECT_ID = :OLD.PROJECT_ID AND EMPLOYEE_ID = :OLD.EMPLOYEE_ID;
        EXCEPTION WHEN NO_DATA_FOUND THEN
          v_project_check := 0;
      END;
      IF(v_project_check = 0) THEN
...
BEGIN
  ...
  IF(v_work_per > 1) THEN
    RAISE_APPLICATION_ERROR(-20505,'TOTAL WORK PERCENTAGE OF EMPLOYEE CANNOT BE MORE THAN 1');
  ELSE
    UPDATE OZU_EMPLOYEES SET TOTAL_WORK_PERCENTAGE = v_work_per;
  END IF;
END;
/*TODO DELETE VE UPDATE DÜZENLENECEK, TASK MI UPDATE INSERT EDILEN PROJE MI ONA BAKILACAK
 */
